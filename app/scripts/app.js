...
app.controller('MainCtrl', ['$scope', '$firebaseSimpleLogin', 'FIREBASE_URI', '$timeout', function ($scope, $firebaseSimpleLogin, FIREBASE_URI, $timeout) {
        ...
        if (error) {
         ...
         toastr.success("Successfully logged in.");
          ...
          if (user.provider === "twitter") {
                ...
                $scope.userImage = user.thirdPartyUserData.profile_image_url; 

          } 
          if (user.provider === "github") {
            $scope.currentUser.email = user.username;
            $scope.userImage = user.thirdPartyUserData.avatar_url;

          }
          if (user.provider === 'password') {
              $scope.userImage = user.md5_hash;
        
          }
          if (user.provider === 'google') {
              $scope.userImage = user.thirdPartyUserData.picture;
          }
    ...
    link: function (scope, element, attrs) {
          attrs.$observe('provider', function(newValue, oldValue) {
              if  (newValue != oldValue) {
                if (scope.provider === 'google' ||  scope.provider === 'github') {
                  attrs.$set('src', scope.userImage);
                }
                else if (scope.provider === 'password') {
                    attrs.$set('src', 'http://gravatar.com/avatar/' + scope.userImage + '.jpg?s=200&r=g');
                }
              }
          })
    }
